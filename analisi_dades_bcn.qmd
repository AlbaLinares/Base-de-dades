...
dades_bcn2 <- dades_bcn %>% 
  select(year,month,bienestar,estres,no2gps_12h,district,µgm3,age_yrs,gender,pm25bcn,BCμg,tmean_12h,humi_12h)

dades_bcn2
```


Notem que el nostre tibble no té la forma desitjada (considera com a variables numèriques o characters allò que nosaltres considerem factors). Canviam els valors de les variables bienestar i estres de un valor numèric a "Baix", "Mitjà" o "Alt" depenent del rang al que pertany.

```{r}
dades_bcn2 <- dades_bcn2 %>%
  na.omit(.) %>% 
  mutate(bienestar = cut(as.numeric(bienestar), breaks =  c(0,3,7,10), labels = c("Bajo","Medio","Alto"), include.lowest = TRUE))%>%
...
-   bienestar: respostes de l'enquesta de benestar (quantitativa). Pot prendre 3 valors: Bajo, Medio o Alto.
...
Facem un test per veure si hi ha correlació positiva entre el nivell de diòxid de nitrogen i els nivells de carboni negre (BC) en micrograms.

```{r}
cor.test(var_quant$no2gps_12h,var_quant$BCμg,alternative="greater")
```

Notem que el p-valor és menor que 0.05 i, per tant, podem asegurar que hi hagi correlació positiva entre aquestes dues variables.
...
Recordem que el nostre estudi volia veure la relació entre la qualitat de vida dels habitants de Barcelona i la qualitat de l'aire que respiren. Vegem si trobem similituds en els perfils de la població:
...
#ordenam per edat, per veure si hi ha similituds
bienestar_estres <- dades_bcn2 %>% 
  arrange(age_yrs) %>% 
  select(bienestar,estres) 

distancia = distance(bienestar_estres)
distancia

#library(philentropy)
#distancia <- distance(bienestar_estres, method = "bhattacharyya")
